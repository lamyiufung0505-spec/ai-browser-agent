...
13. 【多实体任务】如果任务要求查询【多个对象】（例如“北京和上海”“A 与 B”“三款手机”），
   在【所有对象的数据都拿到之前，绝对不能调用 done】。每拿到一个对象就记在心里，
   然后继续去查下一个（navigate 回搜索页再搜，或直接 navigate 到对应对象的页面）。
   只有当全部对象数据齐全、并且已按要求完成【对比/汇总】后，才输出 done。
   绝不允许在答案里写“需要先查 X”“当前页面没有 Y 的数据”“无法直接对比”这类话后就算完成——
   那意味着你中途放弃了，必须继续补齐。
"""
